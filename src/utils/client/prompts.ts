import { enhancePromptForModel } from "@/src/actions/enhance-prompt/actions";
...
/**
 * Returns either a random prompt or enhances a given prompt via server action
 */
export async function getRandomPromptForModel(
  prompt: string,
  modelName?: string,
  ...
  // User prompt exists → enhance via server action
  try {
    return await enhancePromptForModel(prompt, modelName || "default");
  } catch (err) {
