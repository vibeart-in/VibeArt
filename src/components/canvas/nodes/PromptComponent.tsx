...
export function PromptComponent(props: NodeProps<AllNodeTypes>) {
  ...
  if (config.hasModelSelect) {
    return <ModelNode {...props as NodeProps<CheckpointNodeType | LoraNodeType>} config={config} />;
  }
...
function ModelNode({ data, selected, config }: NodeProps<CheckpointNodeType | LoraNodeType> & { config: NodeConfig }) {
...
function PresetNode({ data, selected, config }: NodeProps<PresetsNodeType> & { config: NodeConfig }) {
