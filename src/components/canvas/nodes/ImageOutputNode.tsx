...
import { ImageOutputNodeType } from "@/src/types/canvas/nodeTypes";

export default function ImageOutputNode({ data, selected }: NodeProps<ImageOutputNodeType>) {
  const { zoom } = useViewport();
  const [prompt, setPrompt] = useState(data.prompt || "");
  const [isModelDropdownOpen, setIsModelDropdownOpen] = useState(false);
  const [isEditMode, setIsEditMode] = useState(false);

  // Sync prompt with data.prompt when it changes
  React.useEffect(() => {
