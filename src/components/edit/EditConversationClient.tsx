...
import EditChatView from "./EditChatView";
import MessageSkeleton from "../chat/MessageSkeleton";

export default function EditConversationClient({ conversationId }: { conversationId: string }) {
  ...
  const messageGroups = useMemo(() => {
    ...
    // Ensure each group's turns are sorted oldest → newest
  ...
  return (
    <section className="relative flex h-screen flex-col bg-background text-white">
      <EditChatView messageGroups={messageGroups} />
