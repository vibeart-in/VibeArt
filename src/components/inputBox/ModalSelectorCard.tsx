import { PencilSimple as PencilSimpleIcon, Swap as SwapIcon } from "@phosphor-icons/react";
...
const ModelSelectorCard = React.memo(({ selectedModel, onClick }: ModelSelectorCardProps) => {
  console.log("Rendering ModelSelectorCard"); // For debugging re-renders
  if (!selectedModel) {
    return (
      <div
        onClick={onClick}
  ...
  return (
    ...
    >
      ...
      {selectedModel.cover_image.endsWith(".mp4") ? (
        <video
          key={selectedModel.identifier} // Add a key to force re-mount only when model changes
